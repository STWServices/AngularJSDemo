;function MainCtrl($scope, $http, $templateCache) {
    ...
    //first lets get all the products
    $http({
        method: 'GET',
        url: 'api/product/',
        cache: $templateCache
    ...
    //set the tooltips for all the product images
    $scope.SetToolTip = function (id, name, author, publisher, price, discount, language, year, isbn13, isbn10) {
        ...
        tooltipJS.applyTooltip("imgProduct" + id, content, 5, true);
    };
...
function ProductCtrl($scope, $http, $templateCache, $routeParams) {
...
//Determines if there is any discount for the product or not
